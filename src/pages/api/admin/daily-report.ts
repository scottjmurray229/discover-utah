...
export async function GET(context: APIContext): Promise<Response> {
  ...
  try {
    ...
    const subject = `DU Daily: ${calls} calls, ${fmtCost(cost)}, ${users} users`;
    ...
    const S = {
      ...
      kpiHighlight: 'background:linear-gradient(135deg,rgba(13,115,119,0.15),rgba(9,84,86,0.1));border:1px solid #1E3A5F;border-radius:8px;padding:12px 14px;',
...
<div style="max-width:640px;margin:24px auto;background:#161b22;border:1px solid #30363d;border-radius:12px;overflow:hidden;">
  <div style="background:linear-gradient(135deg,#1E3A5F,#152D4A);padding:20px 24px;">
    <div style="font-size:18px;font-weight:700;color:#fff;">Daily Report</div>
    <div style="font-size:12px;color:rgba(255,255,255,0.7);margin-top:2px;">Discover Utah &middot; Last 24 Hours</div>
  </div>
