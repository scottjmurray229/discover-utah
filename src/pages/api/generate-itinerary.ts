...
function extractDestsFromText(text: string): string[] {
  ...
  const themeMap: Record<string, string[]> = {
    'city': ['salt-lake-city', 'park-city', 'provo'],
    'skyline': ['salt-lake-city'],
    'nightlife': ['salt-lake-city', 'park-city'],
    'nature': ['zion', 'bryce-canyon', 'arches'],
    'wildlife': ['antelope-island', 'bear-lake', 'capitol-reef'],
    'hiking': ['zion', 'bryce-canyon', 'capitol-reef'],
    'beach': ['bear-lake', 'sand-hollow', 'great-salt-lake'],
    'wine': ['park-city', 'salt-lake-city'],
    'art': ['salt-lake-city', 'park-city', 'sundance'],
    'history': ['salt-lake-city', 'capitol-reef', 'dead-horse-point'],
    'museum': ['salt-lake-city', 'provo'],
    'food': ['salt-lake-city', 'park-city', 'provo'],
    'family': ['salt-lake-city', 'arches', 'bryce-canyon'],
    'honeymoon': ['park-city', 'zion', 'sundance'],
    'romantic': ['park-city', 'zion', 'sundance'],
    'budget': ['provo', 'ogden', 'salt-lake-city'],
    'luxury': ['park-city', 'sundance', 'salt-lake-city'],
    'adventure': ['moab', 'zion', 'arches'],
    'lake': ['bear-lake', 'great-salt-lake', 'deer-creek'],
    'waterfall': ['zion', 'provo-canyon', 'ogden'],
    'canyon': ['zion', 'bryce-canyon', 'capitol-reef'],
    'mountain': ['park-city', 'snowbird', 'alta'],
    'ski': ['park-city', 'snowbird', 'alta'],
    'skiing': ['park-city', 'snowbird', 'alta'],
    'music': ['salt-lake-city', 'park-city'],
    'desert': ['moab', 'arches', 'canyonlands'],
    'shopping': ['salt-lake-city', 'park-city'],
    'climbing': ['moab', 'ogden', 'salt-lake-city'],
    'sports': ['salt-lake-city', 'provo', 'ogden'],
    'national park': ['zion', 'bryce-canyon', 'arches', 'canyonlands', 'capitol-reef'],
  };
  ...
  return ['salt-lake-city', 'park-city', 'zion'];
}
...
function buildSystemPrompt(): string {
  return `You are the Discover Utah AI Trip Planner. You generate detailed day-by-day itineraries for Utah travel.
...
- Be specific: real place names, real prices, real transport options (car, shuttle, rideshare, etc.)
- Tag hotel/tour/transport items with affiliateType and affiliateSlotId for future monetization
- affiliateSlotId format: "day{N}-{type}-{destination}" e.g. "day1-hotel-salt-lake-city"
- Utah trips typically require a rental car; Salt Lake City has UTA transit
...
{
  "title": "string — trip title like '7-Day Salt Lake City & National Parks Discovery'",
  "subtitle": "string — budget + month like 'Mid-Range · March 2026'",
  "totalBudget": { "jpy": number, "usd": number },
  "days": [
    {
      "dayNumber": 1,
      "title": "string — day title like 'Arrive in Salt Lake City'",
      "destination": "string — slug like 'salt-lake-city'",
      "items": [
        {
          ...
          "locationName": "string — specific place name for map pin, e.g. 'Temple Square', 'Delicate Arch', 'Angels Landing'. Use real, specific place names."
        }
      ]
    }
  ]
...
function buildUserPrompt(body: RequestBody, destinations: string, duration: string, budgetLevel: string): string {
  ...
  let prompt = `Plan a ${duration}-day Utah trip:\n${parts.join('\n')}`;
