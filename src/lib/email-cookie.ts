 ...
 *
 * If `secret` is empty/undefined (local dev), falls back to accepting plain `dj_email=1`.
 */
export async function verifyEmailCookie(
  ...
  // Parse the dj_email value out of the cookie header
  const match = cookieHeader.match(/(?:^|;\s*)dj_email=([^;]+)/);
